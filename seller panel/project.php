    ...
    if (isset($_POST['add'])) {
        ...
        if (empty($name) || empty($price) || empty($desc) || empty($cat) || empty($image)) {
            ...
            $sql = "INSERT INTO products (name, price, description, category, image) 
                    VALUES ('$name','$price','$desc','$cat','$image')";
            echo ($conn->query($sql)) 
...
if ($page=="manage") {
    ...
    if (isset($_POST['update'])) {
        ...
        elseif(!is_numeric($price)) echo "<p style='color:red;'>Price must be numeric!</p>";
            ...
            $conn->query("UPDATE products SET name='$name',price='$price',description='$desc',category='$cat' WHERE id=$id"); 
...
<tr><th>ID</th><th>Name</th><th>Price</th><th>Category</th><th>Description</th><th>Action</th></tr>
